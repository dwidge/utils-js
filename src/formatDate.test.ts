...
describe("formatDate", () => {
  const testDate = new Date("2024-06-12T12:34:56Z");
  ...
  test("should format date in US format with timezone", () => {
    ...
    expect(formatted).toBe("06/12/2024 - 08:34 am"); // Adjusted to New York timezone
  });

  test("should format date in non-US format with timezone", () => {
    const formatted = formatDate(testDate, "Europe/Paris", "FR");
    expect(formatted).toBe("2024-06-12 - 14:34 pm"); // Adjusted to Paris timezone
  });
